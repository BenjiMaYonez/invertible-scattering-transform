...
# yanglin delete end
    ...
    for coeff in last_coeffs:
        ...
        for (node, j) in zip(corresponding_intermediate_nodes, range(len(psi))):
            j1 = psi[j]['j']
            theta1 = psi[j]['theta']
            #reconstructed_node += ifft(cdgmm(fft(node['coef']), conjugate_transpose(psi[j]['levels'][0])))#F^-1(<F(c), F(mother^H)>)
            reconstructed_node += ifft(cdgmm(fft(node['coef']), psi[j]['levels'][0]))#F^-1(<F(c), F(mother^H)>)
            import torch
            import matplotlib.pyplot as plt
            if max_order == 1:
                plt.imshow(torch.fft.fftshift(torch.fft.ifft2(reconstructed_node[..., 0].relu()).real, dim=(-2, -1)), cmap='gray')
                plt.show()

                plt.imshow(torch.fft.fftshift(torch.fft.ifft2(reconstructed_node[..., 0]).real, dim=(-2, -1)), cmap='gray')
                plt.show()
