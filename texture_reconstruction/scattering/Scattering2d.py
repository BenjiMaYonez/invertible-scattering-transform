...
relative_path = os.path.join(current_dir, '..', '..', '..', 'kymatio-main', 'kymatio')
sys.path.append(relative_path)

from kymatio.torch import Scattering2D as KyScattering2D
...
class Scattering2d(object):
    def __init__(
        ...
        #new code
        self.kymatio_scattering_object = KyScattering2D(J=self.J, L=self.L, shape=(self.M,self.N), max_order=2, model_kind='invertible_scattering').to(torch.device("cuda"))
    ...
    def scattering_coef(
        ...
        #scattering = KyScattering2D(J=J, L=L, shape=(M,N), max_order=2).to(torch.device("cuda"))
        scattering = self.kymatio_scattering_object
        #data = data.cpu()
        data = data.contiguous()
        coefficients = scattering(data)

        S0 = coefficients[:,0,:,:].mean(dim=(-2,-1))

        len_order_1 = J*L
        S1 = coefficients[:,1:1+len_order_1,:,:].mean(dim=(-2,-1))

        len_order_2 = (J*(J-1)//2)*(L**2)
        S2 = coefficients[:,1+len_order_1:,:,:].mean(dim=(-2,-1))
